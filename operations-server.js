...
// Build HTML as string concatenation — avoids ALL backtick/heredoc issues
function buildHTML() {
...
const OperationsCalc = () => {
  ...
  const smartRound = v => {
    ...
    return parseFloat((v - f).toFixed(10)) <= 0.15 ? f : f + 1;
  };
  ...
  const calculate = () => {
    ...
    const res = { numberOfPanels, cutLength, lastPanelWidth: lastPanelDisplay, finalOutput, gatherLabel: gObj.label };
    setResults(res);
    saveToSheet(res);
  };
  ...
  const handlePrint = () => {
    if (!results) return;
    const instDate = inputs.installationDate
      ? new Date(inputs.installationDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })
      : '-';
    ...
    const fabricRows = [
      inputs.mainEnabled   === 'Yes' ? '<tr><td style="padding:8px 14px;font-size:11px;font-weight:600;color:#555;text-transform:uppercase;border-bottom:1px solid #eee;width:38%">Main Fabric</td><td style="padding:8px 14px;font-size:13px;font-weight:700;color:#1A1A2E;border-bottom:1px solid #eee">' + inputs.mainFabric   + '</td></tr>' : '',
      inputs.sheerEnabled  === 'Yes' ? '<tr><td style="padding:8px 14px;font-size:11px;font-weight:600;color:#555;text-transform:uppercase;border-bottom:1px solid #eee">Sheer Fabric</td><td style="padding:8px 14px;font-size:13px;font-weight:700;color:#1A1A2E;border-bottom:1px solid #eee">' + inputs.sheerFabric  + '</td></tr>' : '',
      inputs.liningEnabled === 'Yes' ? '<tr><td style="padding:8px 14px;font-size:11px;font-weight:600;color:#555;text-transform:uppercase">Lining Fabric</td><td style="padding:8px 14px;font-size:13px;font-weight:700;color:#1A1A2E">' + inputs.liningFabric + '</td></tr>' : ''
    ].filter(Boolean).join('');

    const cards = [
      ['Width', inputs.width + '"'], ['Height', inputs.height + '"'],
      ['Installation Date', instDate], ['Panel Type', panelType + '" Panel'],
      ['Stitching Style', inputs.stitchingStyle], ['Gather Style', results.gatherLabel]
    ].map(([l, v]) =>
      '<div style="background:#F8F9FA;border-radius:8px;padding:12px 14px;border-left:3px solid #F0A500;">'
      + '<div style="font-size:9px;color:#888;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:4px">' + l + '</div>'
      + '<div style="font-size:13px;font-weight:700;color:#1A1A2E">' + v + '</div></div>'
    ).join('');

    const lpColor = results.lastPanelWidth === 'None' ? '#999' : '#1A1A2E';

    const html = '<!DOCTYPE html><html><head><meta charset="UTF-8">'
      + '<title>Cutting Sheet - ' + inputs.customerName + '</title>'
      + '<style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:Arial,sans-serif;background:#fff;color:#1A1A2E;}'
      + '@media print{@page{margin:0;size:A4;}body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style>'
      + '</head><body>'
      + '<div style="background:#1A1A2E;padding:22px 32px;display:flex;justify-content:space-between;align-items:center;">'
      +   '<div><div style="color:#F0A500;font-size:22px;font-weight:900;letter-spacing:2px;">D\'MOKSHA HOMES</div>'
      +   '<div style="color:#888;font-size:10px;margin-top:4px;letter-spacing:3px;">EXPRESS YOURSELF. CHOOSE GOODNESS.</div></div>'
      +   '<div style="text-align:right;"><div style="color:#F0A500;font-size:13px;font-weight:700;letter-spacing:1px;">CUTTING INSTRUCTION SHEET</div>'
      +   '<div style="color:#888;font-size:10px;margin-top:4px;">Generated: ' + genDate + '</div></div>'
      + '</div>'
      ...
      +   '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:22px;">' + cards + '</div>'
      + (fabricRows
          ? '<div style="margin-bottom:22px;border-radius:8px;overflow:hidden;border:1px solid #BDD7EE;">'
            + '<div style="background:#2D4A6B;padding:10px 14px;"><span style="color:#fff;font-size:10px;font-weight:700;letter-spacing:2px;">FABRIC INFORMATION</span></div>'
            + '<table style="width:100%;border-collapse:collapse;">' + fabricRows + '</table></div>'
          : '')
      +   '<div style="border-radius:8px;overflow:hidden;border:1px solid #E0E0E0;">'
      +     '<div style="background:#2D4A6B;padding:10px 14px;"><span style="color:#fff;font-size:10px;font-weight:700;letter-spacing:2px;">CUTTING INSTRUCTIONS</span></div>'
      +     '<table style="width:100%;border-collapse:collapse;">'
      +       '<thead><tr style="background:#F0A500;">'
      +         '<th style="padding:10px 14px;text-align:left;font-size:10px;font-weight:700;color:#1A1A2E;">PANEL</th>'
      +         '<th style="padding:10px 14px;text-align:left;font-size:10px;font-weight:700;color:#1A1A2E;">NO. OF PANELS</th>'
      +         '<th style="padding:10px 14px;text-align:left;font-size:10px;font-weight:700;color:#1A1A2E;">CUT LENGTH / PANEL</th>'
      +         '<th style="padding:10px 14px;text-align:left;font-size:10px;font-weight:700;color:#1A1A2E;">LAST PANEL WIDTH</th>'
      +         '<th style="padding:10px 14px;text-align:left;font-size:10px;font-weight:700;color:#1A1A2E;">FINAL OUTPUT</th>'
      +       '</tr></thead>'
      +       '<tbody><tr>'
      +         '<td style="padding:16px 14px;font-weight:700;font-size:14px;color:#1A1A2E;border-bottom:2px solid #F0A500;">' + panelType + '" Panel</td>'
      +         '<td style="padding:16px 14px;font-weight:900;font-size:28px;color:#2D4A6B;border-bottom:2px solid #F0A500;">' + results.numberOfPanels + '</td>'
      ...
      +         '<td style="padding:16px 14px;font-weight:700;font-size:14px;color:' + lpColor + ';border-bottom:2px solid #F0A500;">' + results.lastPanelWidth + '</td>'
      ...
      +       '</tr></tbody></table></div>'
      + '</div>'
      + '<div style="background:#1A1A2E;padding:14px 32px;display:flex;justify-content:space-between;align-items:center;margin-top:16px;">'
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur-sm rounded-2xl shadow-2xl p-6 border border-gray-700">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-amber-400 text-sm font-semibold tracking-wide uppercase">Customer and Window Details</span>
          </div>
          ...
          <div className="space-y-4">
            {[
              ['mainEnabled', 'mainFabric', 'Main Fabric'],
              ['sheerEnabled', 'sheerFabric', 'Sheer Fabric'],
              ['liningEnabled', 'liningFabric', 'Lining Fabric']
            ].map(([enabledKey, nameKey, label]) => (
              <div key={label}>
              ...
              <div className="space-y-0">
                {[
                  ...
                  <div key={lbl} className="flex justify-between items-center py-3 border-b border-gray-600">
                    ...
                    <span className={"font-bold " + (big ? 'text-2xl text-white' : color === 'amber' ? 'text-amber-400' : (val === 'None' ? 'text-gray-500' : 'text-white'))}>
                      {val}
                    </span>
                  </div>
                ))}
...
async function startServer() {
  ...
  const ok = await initializeGoogleSheets();
  if (!ok) console.log('Google Sheets init failed — server will continue');
  app.listen(PORT, () => {
